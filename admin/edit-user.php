...
<?php
  if (isset($_POST['submit'])&&isset($_POST['name'])&&isset($_POST['phone'])&&isset($_POST['uid'])&&isset($_POST['address'])&&isset($_POST['emergency_person'])&&isset($_POST['emergency_phone'])&&isset($_POST['start'])&&isset($_POST['end'])&&isset($_POST['blood_group'])&&isset($_POST['user_group'])) {
    ...
    if(!in_array($extension, $allowed_extension)){
      ...
      move_uploaded_file($_FILES["uploadfile"]["tmp_name"],"Profile/".$filename);
    }

    $picsql = "UPDATE `users` SET `name`= '$name', `phone_no`= '$phone', `address`= '$address', `blood_group`= '$blood_group', `emergency_person`= '$emergency_person', `emergency_phone_no`= '$emergency_phone', `start_time`= '$start', `end_time`= '$end', `access`= '$user_group', image = '$filename' WHERE `uid` = '$uid'";
    if ($conn->query($picsql)) {
        echo "<script>alert('User Updated Successfully!'); window.location.replace('home.php');</script>";
      }
    }
...
?>
  <nav class="navbar navbar-expand navbar-dark bg-dark static-top">
    ...
    <a class="navbar-brand mr-1" href="home.php"><img src="../images/HiddenBurger.jpg" class="image image-responsive" height="45" width="45"> The Hidden Burger</a>
            ...
            <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
              <div class="form-row">
                ...
                <div class="form-group col-md-3">
                  ...
                  <image src="Profile/<?php echo $user['image'];?> " width="200" height="200"></td>
                  <input type="file" accept="image" class="form-control" name="uploadfile" id="image" placeholder="image">
                </div>
